...
class LinearStream(nn.Module):
    ...
    MLP-based network without activation functions to capture linear features.
    ...
    def __init__(self, seq_len: int, pred_len: int):
        ...
        # Two linear blocks with avg pooling (from xPatch)
        hidden1 = seq_len // 2
        hidden2 = hidden1 // 2
        
        self.block1 = nn.Sequential(
            nn.Linear(seq_len, hidden1),
        ...
        # Final expansion
        self.output = nn.Linear(hidden2, pred_len)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        x = self.block1(x)
        # Handle potential size mismatch from AvgPool1d
        x = x[..., :x.shape[-1]//2*2]  
        x = x[..., ::2]  # Downsample
...
class NonLinearStreamHybrid(nn.Module):
    ...
    def __init__(
        ...
        if use_multiscale:
            ...
            # Original xPatch single depthwise conv
            self.depthwise = nn.Sequential(
                nn.Conv1d(
                    self.num_patches, self.num_patches,
                    kernel_size=patch_len, stride=patch_len, groups=self.num_patches
                ),
